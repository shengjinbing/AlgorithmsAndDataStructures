 ...
 *
 * 9.HashMap的hash算法和扩容机制的原因？？？？？resize()https://blog.csdn.net/lkforce/article/details/89521318
 *   1.之前一条链现在分成两条链表
 *     正常情况下，计算节点在table中的下标的方法是：hash&(oldTable.length-1)，扩容之后，table长度翻倍，计算table下标的方法是
 *     hash&(newTable.length-1)，也就是hash&(oldTable.length*2-1)，于是我们有了这样的结论：这新旧两次计算下标的结果，要不然就相同
 *     ，要不然就是新下标等于旧下标加上旧数组的长度。
 *   2.当前容量的大小是否小于64，如果小于这个值还是要进行扩容而不是转化为红黑树
 *10.SparseArray和HashMap的区别?
 *11.hashmap 实现原理，给一个 key 如何计算槽位，如何取值?
 *
 * 红黑树阀值为什么是8和6？
 * 1.红黑树虽然查询效率比链表高，但是结点占用的空间大
 * 2.根据树化的注解得来，大概意思就是：如果 hashCode的分布离散良好的话，那么红黑树是很少会被用到的，因为各个值都均匀分布，
 * 很少出现链表很长的情况。在理想情况下，链表长度符合泊松分布，各个长度的命中概率依次递减，注释中给我们展
 * 示了1-8长度的具体命中概率，当长度为8的时候，概率概率仅为0.00000006，这么小的概率，HashMap的红黑树
 * 转换几乎不会发生，因为我们日常使用不会存储那么多的数据，你会存上千万个数据到HashMap中吗？
 * 3.但是红黑树退化为链表的阈值却是6，为什么不是小于8就退化呢？比如说7的时候就退化，偏偏要小于或等于6？
 *   主要是一个过渡，避免链表和红黑树之间频繁的转换。如果阈值是7的话，删除一个元素红黑树就必须退化为链表，
 *   增加一个元素就必须树化，来回不断的转换结构无疑会降低性能，所以阈值才不设置的那么临界。
 *
 * SynchronizedMap
 * 对自己每个方法加锁
 *
